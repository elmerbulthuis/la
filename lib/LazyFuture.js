	...
	function definer(Future){
		...
		function LazyFuture(){
				...
				dependencies.forEach(function(dependency, dependencyIndex){
					if(Array.isArray(dependency)){
						resolveDependencies(dependency, function(err, value){
							...
							if(!countdown--) cb(firstError, results);
						});
					}
					else{
			...
			function getValue(cb){
				if(cb.length != 1 && cb.length != 2) throw 'callback should have one (value) or two (err, value) arguments'
				...
				resolveDependencies(dependencies, function(err, results){
					...
					resolveResult = resolver.apply(this, results.concat([function(){
						switch(arguments.length){
							...
							case 2:
							if(arguments[0]) return flushCallbacks(getValueQueue, arguments[0], null);
							cachedValue = arguments[1];
							break;
						}

						resolveState = 2;
						future.emit('resolved');
